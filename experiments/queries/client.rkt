...
(require profile
         (prefix-in gv: profile/render-text))
...
(define (execute-insert use-sdb?)
  (let-values ([(low high) (random-range 20)]
               [(ntimes) 10])
    (let ([v (where (open-view DBPATH "test") (format "value >= ~a and value <= ~a" low (+ high 10)))])
      (begin
        (repeater
         (lambda ()
           (if use-sdb?
               (insert v "id, value" (list high (+ high 5)))
