...
(define (make-get-join-contracts/c ctc details)
  (define (view-contract/c in-ctc out-ctc)
    (make-contract
     ...
     (λ (b)
       (λ (vc)
         (λ (out/c in/c)
           (vc (and/c out-ctc out/c) (λ (v1 v2 jc) (and (in-ctc v1 v2 jc) (in/c v1 v2 jc)))))))))
  ...
  (cond [(= dl 4)
        ...
        [else
         (view-contract/c (λ (v1 v2 jc) #t) ctc)]))
...
(define (build-view-pair v1 v2)
  ...
  (define (get-join-contracts v) ((shill-view-get-join-contracts v) any/c (λ (v1 v2 jc) #t)))
  ...
  (define/contract (join v1 v2 jcond post)
    (->i ([v1 shill-view?]
          ...
          [jcond (v1 v2) (and/c string?
                                (λ (jc) (v1-in/c v1 v2 jc))
                                (λ (jc) (v2-in/c v2 v1 jc)))]                                               
          [post procedure?])
         [result any/c])
...
; (view/c (fetch/p (where "x < 10"))
  ...
  (define/contract v1 (view/c #:fetch (list "fetch" #t)
                              ...
                              #:where (list "where" #t)
                              #:join (list "join" #t (λ (v) v) fk-pred))
                              ;#:join (list "join" #t (λ (v) v) (λ (p) (displayln p))))
    (open-view "test.db" "artist"))
  (define v2 (open-view "test.db" "track"))
